...
//	и os - не удаляйте их из импорта. Скорее всего, вам понадобится пакет "fmt", но вы можете использовать любой
//	другой пакет для записи в стандартный вывод не удаляя fmt.
//
...
func readTask1() (value1, value2, operation interface{}) {
	var values [3]interface{}
...
func checkFloat64(i interface{}) bool {
	switch i.(type) {
	case float64:
		return true
	default:
...
func main() {
	value1, value2, operation := readTask1() // исходные данные получаются с помощью этой функции
	// все полученные значения имеют тип пустого интерфейса

	if !checkFloat64(value1) {
		return
	}
	if !checkFloat64(value2) {
		return
	}
